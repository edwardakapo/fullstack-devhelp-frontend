...
export default function PostCards(props) {
    ...
    const savePost = () => {
      axios.patch(`${process.env.REACT_APP_SERVER_ENDPOINT}/post/${props.id}/save`, {} , { withCredentials : true })
        .then((response) => {
    ...
    const unsavePost = () => {
      axios.patch(`${process.env.REACT_APP_SERVER_ENDPOINT}/post/${props.id}/unsave`, {} , { withCredentials : true })
        .then((response) => {
...
const handleSubmit = (event) => {
  ...
  const text = comment
  axios.patch(`${process.env.REACT_APP_SERVER_ENDPOINT}/post/${props.id}/comment`, { text }, {withCredentials : true})
  .then((response) => {
            ...
            <div className=' items-start'>
              ...
              {props.isDelete && (
                    <Button size="large" sx={{p:3}} onClick={() => {
                      axios.delete(`${process.env.REACT_APP_SERVER_ENDPOINT}/post/${props.id}/delete`, {withCredentials : true})
                      .then((response) => {
